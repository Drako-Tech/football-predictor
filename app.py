...
def calculate_league_table(df, predict_home=None, predict_away=None, ph_goals=0, pa_goals=0):
    # Copy dataframe to preserve original data state
    working_df = df.copy()
    ...
    # Home stats aggregation via vectorized grouping operators
    ...
    # Away stats aggregation via vectorized grouping operators
    ...
    # Join and synthesize global team parameters
    ...
    # Vectorized Form string calculator mapping arrays
    ...
    # Tie-Breaker Sorting Logic Framework: Points -> Goal Difference -> Goals Scored -> Alphabetical
    table = table.sort_values(by=['Points', 'GD', 'GF', 'Team'], ascending=[False, False, False, True]).reset_index(drop=True)
    table.index += 1 # Position index numbering
...
if uploaded_file is not None:
    try:
        ...
        if all(col in df.columns for col in required_cols):
            ...
            # --- TABULAR WORKSPACE NAVIGATION NAVIGATION ---
            ...
            with tab1:
                ...
                # Interactive Contextual Multipliers Sidebar Panel
                ...
                if st.button("Run Simulation Engine", type="primary"):
                    ...
                    # Core Derivative Betting Markets Calculations
                    ...
                    # Store variables in session state to pass to the League Table Tab
            ...
            with tab2:
                ...
                st.caption("This system calculates real-time league rankings using comprehensive performance variables and goal differential tie-breakers directly extracted from your dataset file columns [1].")
                
                # Live Point Projections Toggle
